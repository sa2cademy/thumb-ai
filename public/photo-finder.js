...
const KNOWN_ENTITY_RULES = [
  {
    ...
    aliases:['bts','방탄','방탄소년단'],
    mustMatchTokens:['bts','방탄','방탄소년단'],
    preferredShot:'단체 공식 사진',
    queries:['BTS 단체사진','BTS official group photo','BTS full group photo','방탄소년단 공식 단체사진','BTS 무대 단체사진']
  },
  {
    label:'넷플릭스', type:'brand',
    aliases:['넷플릭스','netflix'],
    mustMatchTokens:['넷플릭스','netflix'],
    preferredShot:'본사 또는 공식 행사 사진',
    queries:['넷플릭스 본사','Netflix headquarters','넷플릭스 공식 행사 사진','Netflix office building','Netflix brand event']
  },
  {
    label:'태극기', type:'symbol',
    aliases:['태극기','대한민국 국기','korea flag'],
    mustMatchTokens:['태극기'],
    preferredShot:'태극기 실사 사진',
    queries:['태극기 사진','한국 국기 실사','태극기 펄럭이는 사진','Korea national flag photo']
  },
  {
    label:'한국', type:'country',
    aliases:['한국','대한민국','korea'],
    mustMatchTokens:['한국','대한민국','korea'],
    preferredShot:'대표 상징 사진',
    queries:['한국 상징 사진','Korea landmark photo','대한민국 랜드마크 사진','한국 도시 전경']
  },
  {
    label:'케이팝 데몬 헌터스', type:'title',
    aliases:['케데헌','케이팝 데몬 헌터스','kpop demon hunters','k-pop demon hunters'],
    mustMatchTokens:['케이팝','데몬','헌터스'],
    preferredShot:'공식 스틸컷',
    queries:['케이팝 데몬 헌터스 스틸컷','KPop Demon Hunters official still','케데헌 공식 이미지','KPop Demon Hunters poster']
  }
];
...
const BANNED_HINTS        = [
  '차트','그래프','인포그래픽','통계','보고서','리포트','자료','기사','뉴스','속보',
...
const WATERMARK_HINTS     = [
  ...
  'clipart','bigstock','pond5','fotolia','corbis','masterfile','canstock','superstock'
];
const BLOCKED_IMG_DOMAINS = new Set([
...
function isBlockedDomain(url) {
  try {
    ...
    return BLOCKED_IMG_DOMAINS.has(host) || [...BLOCKED_IMG_DOMAINS].some(d => host.endsWith('.'+d));
...
function buildSentencePlan(index, sentences, representativeKeyword, gPlan, facePriorityLock) {
  ...
  const repEntity   = detectEntityFromText(representativeKeyword || '', gPlan.anchor_entities, false)
                   || buildRawEntity(representativeKeyword || gPlan.primary_entity || '', 'general');
  const useRepFallback = shouldUseRepresentativeFallback(sentence, localEntity, representativeKeyword);
  const target = useRepFallback
    ? (repEntity   || localEntity || buildRawEntity(gPlan.primary_entity || '핵심 대상', 'general'))
    : (localEntity || repEntity   || buildRawEntity(gPlan.primary_entity || '핵심 대상', 'general'));
  ...
  const searchQueries  = uniqueStrings([
    ...buildQueriesForEntity(target, sentence, facePriorityLock),
    ...(useRepFallback ? buildQueriesForEntity(repEntity, sentence, facePriorityLock) : [])
  ]).slice(0, MAX_QUERY_COUNT);

  return {
    scene_role: sceneRole,
    target_entity: target.label || representativeKeyword || gPlan.primary_entity || '핵심 대상',
    entity_type: target.type || 'general',
    visual_intent: visualIntent,
    must_match_tokens: mustMatchTokens,
    search_queries: searchQueries,
    reasoning_short: useRepFallback
      ? `설명 문장이라 대표 키워드 ${target.label || representativeKeyword} 사진으로 맞췄습니다.`
      : `${target.label}가 바로 드러나는 사진만 우선합니다.`,
...
function buildRawEntity(label, type) {
  ...
  return {
    label:safeLabel, type:type||'general',
    mustMatchTokens:tokenize(safeLabel).slice(0,4),
    queries:[]
  };
}
...
function inferTypeFromLabel(label) {
  ...
  if (/케데헌|데몬 헌터스/.test(t))    return 'title';
  // 한글 2자 이상 단어는 인물명으로 추정
...
function buildQueriesForEntity(entity, sentence, facePriorityLock) {
  ...
  if (entity.type==='group') {
    base.push(`${label} 단체사진`, `${label} official group photo`, `${label} full group photo`, `${label} 무대 단체사진`);
  } else if (entity.type==='person') {
    // 인물은 실제 얼굴 사진 위주 쿼리
    base.push(`${label}`, `${label} 사진`, `${label} 인물사진`, `${label} 프로필 사진`, `${label} photo`);
    if (facePriorityLock) base.push(`${label} 공식 화보`, `${label} 행사 사진`);
  } else if (entity.type==='brand') {
    base.push(`${label} 본사`, `${label} headquarters`, `${label} 공식 행사 사진`, `${label} logo building`);
  } else if (entity.type==='symbol') {
    base.push(`${label} 사진`, `${label} 펄럭이는 사진`, `${label} high quality`);
  } else if (entity.type==='country') {
    base.push(`${label} 상징 사진`, `${label} landmark photo`, `${label} 대표 이미지`);
  } else if (entity.type==='title') {
    base.push(`${label} 공식 스틸컷`, `${label} official still`, `${label} poster`);
  } else {
    base.push(`${label}`, `${label} 사진`, `${label} official photo`);
  }

  if (/케이팝|k-pop/i.test(sentence) && entity.type!=='brand') {
    base.push(`${label} 공연 사진`, `${label} performance photo`);
  }
...
대표 키워드: ${representativeKeyword || '(없음)'}
컨텍스트: ${context || '(없음)'}
대본:
${(() => {
    const MAX_LINES = 40;
    if (sentences.length <= MAX_LINES) return sentences.map((s,i) => `${i+1}. ${s}`).join('\n');
...
JSON 형식:
{
  ...
  "rule_summary": "짧은 한 줄"
    ...
    const data = await callClaudeForJson(prompt, anthropicKey, 500);
    return {
      ...
      primary_entity: representativeKeyword
                    || ensureStringArray(data?.anchor_entities, fallback.anchor_entities, 1)[0]
                    || fallback.primary_entity
    };
  } catch(e) {
    console.warn('analyzeWholeScript fallback', e);
    return fallback;
  }
}
...
function buildFallbackGlobalPlan(sentences, context, representativeKeyword) {
  ...
  return {
    overall_tone:   representativeKeyword ? `${representativeKeyword} 중심 설명형` : '대표 대상 중심 설명형',
    anchor_entities:extracted,
    rule_summary:   '대표 키워드 우선, 문장 속 명확한 대상 우선, 랜덤 얼굴/문서/기사형 이미지는 제외',
    primary_entity: representativeKeyword || extracted[0] || '핵심 대상'
  };
}
...
function extractEntitiesFallback(sentences, context, representativeKeyword) {
  ...
  KNOWN_ENTITY_RULES.forEach(rule => {
    const text = `${context||''} ${(sentences||[]).join(' ')}`.toLowerCase();
    if (rule.aliases.some(a => text.includes(a.toLowerCase()))) values.push(rule.label);
  });
...
async function searchByPriorityQueries({ queries, plan, targetCount, previousStarts={}, cardIndex }) {
  ...
  const queryStarts = {...previousStarts};
  const collected   = [];

  for (const query of safeQueries) {
    let start = Number(queryStarts[query] || 1);
    while (start <= MAX_QUERY_PAGE && collected.length < targetCount) {
      let items;
      try { items = await searchImages(query, start); }
      catch(e) { console.warn('search error', e); break; }

      const filtered = items
        .map(item => evaluateCandidate(item, query, plan, cardIndex))
        .filter(Boolean)
      ...
      if (items.length < 2) break; // 결과 거의 없으면 다음 쿼리로
    }
    if (collected.length >= targetCount) break;
  }

  return {
    photos:    uniquePhotos(collected).sort((a,b) => b.score-a.score).slice(0, Math.max(targetCount, SECOND_PASS_TOP_K)),
    queryStarts
  };
}
...
function evaluateCandidate(item, query, plan, cardIndex) {
  ...
  if (!key) return null;
  if (width > 0 && width < MIN_IMAGE_SIZE)   return null; // 크기 있을 때만 필터 (0=미확인은 통과)
  ...
  if (plan.entity_type === 'group') {
    ...
    const hasSoloOnly    = BTS_SOLO_HINTS.some(h => haystack.includes(h))
                        && !haystack.includes('bts') && !haystack.includes('방탄');
...
async function secondPassRerankPhotos(photos, plan, anthropicKey) {
  ...
  if (unique.length < SECOND_PASS_MIN_POOL || !anthropicKey) return unique;
...
JSON 형식: {"ordered_indices":[1,2,3,4,5,6,7,8]}`;
    ...
    const data    = await callClaudeForJson(prompt, anthropicKey, 300);
    ...
    const used      = new Set();
    ordered.forEach(idx => {
      const photo = top[idx-1];
      if (!photo) return;
      const key = photo.link || photo.thumbnail;
      if (used.has(key)) return;
      reordered.push({ ...photo, score: photo.score + 38 - reordered.length * 4 });
      used.add(key);
    });
    top.forEach(photo => {
      const key = photo.link || photo.thumbnail;
      if (!used.has(key)) reordered.push(photo);
    });
    return [...reordered, ...unique.slice(SECOND_PASS_TOP_K)].sort((a,b) => b.score-a.score);
  } catch(e) {
    console.warn('second pass fallback', e);
    return unique;
  }
}
...
function findNextAvailablePhotoIndex(cardIndex, startIndex, excludeIdx=-1) {
  ...
  for (let i = startIndex; i < state.photos.length; i++) {
    ...
    const key = state.photos[i].link || state.photos[i].thumbnail;
    if (key && !selectedPhotoKeys.has(key)) return i;
  }
  return -1;
}
...
function uniquePhotos(photos) {
  const map = new Map();
  (photos||[]).forEach(p => {
    const key = p.link || p.thumbnail;
    if (!key) return;
    const prev = map.get(key);
    if (!prev || Number(p.score||0) > Number(prev.score||0)) map.set(key, p);
  });
  return [...map.values()];
}
